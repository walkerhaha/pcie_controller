...
//
// Filename    : DWC_pcie_ctl_bcm41.v
// Revision    : $Id: //dwh/pcie_iip/main/fairbanks/design/common/DWC_pcie_ctl_bcm41.sv#1 $
// Author      : Liming SU    12/27/18
// Description : DWC_pcie_ctl_bcm41.v Verilog module for DWC_pcie_ctl
//
// DesignWare IP ID: a95d4319
//
////////////////////////////////////////////////////////////////////////////////



module DWC_pcie_ctl_bcm41 (
    clk_d,
    rst_d_n,
    data_s,
    data_d
`ifdef DWC_SYNC_TYPE_1_SRC
  `define DWC_NEW_SYNC_TECH_MAP
`else
  `ifdef DWC_SYNC_TYPE_2_SRC
    `define DWC_NEW_SYNC_TECH_MAP
  `else
    `ifdef DWC_SYNC_TYPE_3_SRC
      `define DWC_NEW_SYNC_TECH_MAP
    `else
      `ifdef DWC_SYNC_TYPE_4_SRC
        `define DWC_NEW_SYNC_TECH_MAP
      `endif
    `endif
  `endif
...
// SMD: Synthesis directive 'dc_tcl_script_end' is not recognized for pragma 'synopsys'
// SJ: Temporarily suppressed due to Spyglass's limitatin.
`ifndef DWC_EMB_SCR_IS_FOR_FC
  // synopsys dc_tcl_script_begin
  // for {set i 1} {$i <= 4} {incr i} { if {[get_cells -quiet "GEN_FST$i.*U_sync_$i*"] != {}} { set_size_only [get_cells "GEN_FST$i.*U_sync_$i*"] "true"; set_dont_retime [get_cells "GEN_FST$i.*U_sync_$i*"] "true"; break } }; unset i;#
...
`else
// Default to report all the message
  localparam BCM_MSG_VERBOSITY = 255;
`endif
...
`ifdef DW_REPORT_SYNC_PARAMS
  initial begin
    if ((F_SYNC_TYPE & 7) > 0)
      $display("Information: *** Instance %m is configured as follows: WIDTH is: %0d, F_SYNC_TYPE is: %0d ***", WIDTH, (F_SYNC_TYPE & 7));
  end
`endif
`endif
`endif
...
`else
  `ifdef DW_MODEL_MISSAMPLES
  if ((BCM_MSG_VERBOSITY&1) == 1) begin
  initial begin
...
`endif
  ...
  generate
    ...
    if (SVA_TYPE_BIT1BIT0 == 1) begin : GEN_SVATP_EQ_1
      ...
      DWC_pcie_ctl_sva01 #(WIDTH, (F_SYNC_TYPE & 7), VERIF_EN) P_SYNC_HS (.*);
    end
    ...
    `endif
      ...
      DWC_pcie_ctl_sva05 #(WIDTH, (F_SYNC_TYPE & 7)) P_SYNC_GC (.*);
    end

    if (SVA_TYPE_BIT2 == 0) begin : GEN_SVATP_BIT2_EQ_0
      DWC_pcie_ctl_sva08 #(WIDTH) P_SYNC_EQ_RST (.*);
    end
  endgenerate
  `else
    `ifdef DW_MODEL_MISSAMPLES
