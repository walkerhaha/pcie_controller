...
//
// Filename    : DWC_pcie_ctl_bcmmod86.v
// Revision    : $Id: //dwh/pcie_iip/main/fairbanks/design/common/DWC_pcie_ctl_bcmmod86.sv#1 $
// Author      : Rick Kelly/WCYU/LMS 12/03/14
// Description : DWC_pcie_ctl_bcmmod86.v Verilog module for DWC_pcie
//
// DesignWare IP ID: 5366b8e5
//
////////////////////////////////////////////////////////////////////////////////

module DWC_pcie_ctl_bcmmod86 (
    clk,
    rst_n,
    init_n,
    push_req_n,
...
// Improving high fanout in DWC_pcie_ctl_bcm86.sv
...
//assign advance_wr_addr = ~(push_req_n | (full_int & pop_req_n));
  assign advance_wr_addr = ~push_req_n;
...
`ifdef SYNTHESIS
localparam REG_DELAY  = 0;
`else
localparam real REG_DELAY  = (1.0 * ((REG_DELAY_TIME  <  0)? 0 : REG_DELAY_TIME )) /
                                    ((REG_DELAY_DENOM <= 0)? 1 : REG_DELAY_DENOM);
`endif
...
// SJ: The design incorporates delays for behavioral simulation. Timescale compiler directive is assumed to be defined in the test bench.
  always @ (posedge clk or negedge rst_n) begin : registers_PROC
    if (rst_n == 1'b0) begin
      empty_n          <= #REG_DELAY 1'b0;
      almost_empty_n   <= #REG_DELAY 1'b0;
      half_full_int    <= #REG_DELAY 1'b0;
      almost_full_int  <= #REG_DELAY 1'b0;
      full_int         <= #REG_DELAY 1'b0;
      error_int        <= #REG_DELAY 1'b0;
      wr_addr_int      <= #REG_DELAY {ADDR_WIDTH{1'b0}};
      rd_addr_at_max   <= #REG_DELAY 1'b0;
      wr_addr_at_max   <= #REG_DELAY 1'b0;
      rd_addr_int      <= #REG_DELAY {ADDR_WIDTH{1'b0}};
      word_count       <= #REG_DELAY {ADDR_WIDTH{1'b0}};
...
// SJ: This module can be specifically configured/implemented with only a synchronous reset or no resets at all.
      empty_n          <= #REG_DELAY next_empty_n;
      almost_empty_n   <= #REG_DELAY next_almost_empty_n;
      half_full_int    <= #REG_DELAY next_half_full;
      almost_full_int  <= #REG_DELAY next_almost_full;
      full_int         <= #REG_DELAY next_full;
      error_int        <= #REG_DELAY next_error;
      rd_addr_at_max   <= #REG_DELAY next_rd_addr_at_max;
      wr_addr_at_max   <= #REG_DELAY next_wr_addr_at_max;
      wr_addr_int      <= #REG_DELAY next_wr_addr;
      rd_addr_int      <= #REG_DELAY next_rd_addr;
      word_count       <= #REG_DELAY next_word_count;
